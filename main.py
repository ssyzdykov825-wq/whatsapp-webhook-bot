...
def process_new_lead(name, phone):
    ...
    Теперь учитывает статус заказов в CRM: если есть заказ в статусе 1 ("в обработке"), новый не создаём.
    """
    if client_in_db_or_cache(phone):
        print(f"⚠️ Клиент {phone} уже в базе/кэше (в process_new_lead), пропускаем создание/обновление.")
        return None 

    # Проверяем CRM через обновлённый client_exists
    crm_status = client_exists(phone)
    print("DEBUG: результат client_exists =", crm_status)  # 🔎 отладка

    if crm_status == 1:
        print(f"⚠️ У клиента {phone} уже есть заказ в статусе 1 (в обработке). "
              "Не создаём новый заказ, но сохраняем клиента в БД.")
        save_client_state(phone, name=name, in_crm=True)
        return None

    # Если заказов нет или они не в статусе 1 → можно создавать
    print(f"✅ У клиента {phone} нет активных заказов. Создаем новый заказ в CRM.")
    order_id = create_order(name, phone)

    print("DEBUG: ответ CRM при создании заказа =", order_id)  # 🔎 отладка
    ...
    if order_id:
        ...
        print(f"❌ Не удалось создать заказ для {name}, {phone}. "
              "Создаем запись клиента без CRM связи в боте.")
