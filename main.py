...
from salesrender_api import create_order, get_lead_status
...
def process_new_lead(name, phone):
    """
    Основная логика, которая решает, нужно ли создавать новый заказ.
    Использует get_lead_status для проверки текущего статуса лида в CRM.
    """
    print(f"🔎 Проверяем CRM для номера {phone}...")
    # Используем новую функцию, которая возвращает статус или None
    existing_lead = get_lead_status(phone)

    if existing_lead:
        print(f"➡️ Клиент найден в CRM. Проверяем его статус: {existing_lead['statusId']}")
        # Если статус НЕ равен 1 (статус 'Новый' в SalesRender), создаем новый заказ
        if existing_lead['statusId'] != 1:
            print(f"⚠️ Его лид в обработке (статус {existing_lead['statusId']}). Создаем новый заказ.")
            order_id = create_order(name, phone)
            if order_id:
                print(f"✅ Заказ {order_id} успешно создан.")
            else:
                print(f"❌ Не удалось создать заказ.")
            return order_id
        else:
            # Если статус равен 1, то лид уже есть и он новый, поэтому не создаем дубль
...
def process_salesrender_order(order):
    ...
    try:
        ...
        if "customer" in order:
            ...
            # Добавьте вашу функцию нормализации телефона
        ...
        # ... (остальная логика для process_salesrender_order)
        # Если ваш код для этой функции не меняется, его можно оставить как есть
