...
def get_gpt_response(user_message):
    try:
        response = client.chat.completions.create(
            model="gpt-4o",
            messages=[
                {"role": "system", "content": "Ты эксперт по улучшению зрения. Ты помогаешь людям после неудачного звонка узнать о натуральном средстве Healvix, которое восстанавливает зрение. Продукт безопасен, без побочек, подходит людям 35+. В основе — экстракт черники, лютеин, таурин. Можно принимать с любыми лекарствами. Не требует окрашивания линз и не вызывает зависимости. Бот отвечает как человек, мягко подталкивает к заказу, использует эмпатию, не давит."},
