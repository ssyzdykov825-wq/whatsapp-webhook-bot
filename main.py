...
from salesrender_api import create_order, get_orders_by_phone
...
def process_new_lead(name, phone):
    ...
    - Если заказов нет или все они в статусах 3,4,8,10,11 → создаем новый заказ.
    """

    # Проверка локальной базы/кэша
    if client_in_db_or_cache(phone):
        print(f"⚠️ Клиент {phone} уже в базе/кэше (process_new_lead), пропускаем создание/обновление.")
        return None

    try:
        orders = get_orders_by_phone(phone)  # вытягиваем список заказов клиента
        print(f"🔍 Заказы по {phone}: {orders}")

        # Если есть заказ со статусом 1 — новый не создаем
        if any(order.get("statusId") == 1 for order in orders):
            print(f"⚠️ У клиента {phone} есть заказ в статусе 1 → новый заказ не создаём.")
            save_client_state(phone, name=name, in_crm=True)
            return None

        # Если заказов нет или только в «разрешённых» статусах → создаем новый
        allowed_statuses = [3, 4, 8, 10, 11]
        if not orders or all(order.get("statusId") in allowed_statuses for order in orders):
            print(f"✅ У клиента {phone} либо нет заказов, либо все в статусах {allowed_statuses}. Создаем новый заказ.")
        ...
        # Если есть заказы, но они не подходят под условия — ничего не делаем
        print(f"⚠️ У клиента {phone} заказы есть, но их статусы не позволяют создать новый.")
        save_client_state(phone, name=name, in_crm=True)
        return None
