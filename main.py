...
def process_new_lead(name, phone):
    ...
    Логика:
      - Если клиента нет в базе бота → проверяем CRM.
      - Если клиента нет в CRM → создаём заказ.
      - Если клиент есть в CRM → создаём заказ только если последний заказ имеет разрешённый статус.
    """
    if client_in_db_or_cache(phone):
        print(f"⚠️ Клиент {phone} уже в базе/кэше (в process_new_lead), пропускаем создание/обновление.")
        return None

    # Получаем последний заказ по номеру телефона
    ...
    if last_order:
        ...
        print(f"ℹ️ Найден последний заказ для {phone} со статусом: {status}")

        if status in allowed_statuses:
            print(f"♻️ Статус {status} разрешает создание нового заказа.")
            ...
            if order_id:
                ...
                print(f"❌ Ошибка при создании заказа для {name}, {phone}.")
                save_client_state(phone, name=name, in_crm=True)
                return None
        else:
            print(f"⏳ Клиент {phone} уже в CRM со статусом {status}, новый заказ не создаём.")
            save_client_state(phone, name=name, in_crm=True)
            return None
    else:
